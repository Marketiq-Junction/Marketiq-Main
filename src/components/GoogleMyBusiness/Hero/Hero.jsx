...
const Hero = () => {
  ...
  const handleWhatsAppClick = () => {
    const message =
      "Hello, I would like to know more about Google My Business Management.";
    const encodedMessage = encodeURIComponent(message);
    const whatsappURL = `https://wa.me/${whatsappNumber}?text=${encodedMessage}`;
    window.open(whatsappURL, "_blank");
  };
  ...
  return (
    <section className="flex flex-col md:flex-row justify-center items-center py-10 sm:py-14 bg-[#4A9BD3]">
      <div className="flex flex-col md:flex-row max-w-6xl w-full gap-8 px-4 sm:px-8 lg:px-12">
        {/* Left Content */}
        <motion.div
          className="flex-1 flex flex-col justify-center"
        ...
        >
          ...
          {/* Input + Button */}
          <motion.div
            className="relative w-full mt-6 flex flex-col sm:flex-row sm:items-center sm:gap-3"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, delay: 0.4 }}
          >
            <input
              type="text"
              placeholder="Enter Your Website"
              className="flex-1 py-3 px-4 sm:py-4 sm:px-5 border border-gray-300 text-gray-700 focus:outline-none focus:ring-2 focus:ring-teal-500 text-sm sm:text-base"
            />
            <button
              onClick={() => {
                if (
              ...
              className="mt-3 sm:mt-0 bg-[#50C3C6] text-white px-4 sm:px-6 py-3 hover:bg-teal-600 focus:outline-none focus:ring-2 focus:ring-teal-400 focus:ring-offset-2 text-sm sm:text-base"
            >
              Request a Audit
            </button>
          </motion.div>
