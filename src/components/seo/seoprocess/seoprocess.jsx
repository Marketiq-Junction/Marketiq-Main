...
const SEOProcess = () => {
  ...
  return (
    <section className="bg-gray-50 py-16 shadow-lg mb-8">
      <div className="container mx-auto px-4 lg:px-16">
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-16 relative">
          {steps.map((step) => (
            <div key={step.id} className="relative">
              <button
                className={`bg-[#50C3C6] text-black font-semibold text-center py-6 px-4 rounded-lg shadow-md w-44 md:w-48 transition-all duration-200 hover:scale-105 hover:shadow-xl ${
                  activeStep.id === step.id ? "bg-blue-400 text-black" : ""
                }`}
              ...
              {/* Active Arrow */}
              {activeStep.id === step.id && (
                <div className="absolute left-1/2 transform -translate-x-1/2 top-full w-0 h-0 border-l-[10px] border-l-transparent border-r-[10px] border-r-transparent border-t-[10px] border-t-blue-400 md:block"></div>
              )}
            </div>
          ))}
        </div>
