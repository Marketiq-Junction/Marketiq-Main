...
const Advantages = () => {
  ...
  return (
    <section
      ref={ref}
      className="bg-gradient-to-b from-cyan-200 to-blue-200 py-16"
    >
      <div className="container mx-auto px-6 lg:px-16 text-center">
        <motion.h1
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {advantages.map((advantage, index) => (
            <motion.div
              key={index}
              className="bg-white shadow-md rounded-lg p-6 text-left flex flex-col justify-start transition-transform transform hover:scale-105"
              initial={{ opacity: 0, y: 50 }}
              animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.5, delay: index * 0.2 }}
            >
              <h3 className="text-lg sm:text-xl font-semibold text-blue-600 mb-4 text-center md:text-left">
                {advantage.title}
              </h3>
