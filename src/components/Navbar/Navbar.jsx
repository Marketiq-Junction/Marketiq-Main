...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (dropdownOpen || mobileDropdownOpen) {
        // Close the dropdowns if user clicks outside
        closeDropdowns();
      }
    };
  ...
  return (
    <div className="w-full h-[10vh]">
      ...
      {/* Mobile Logo - Full Width, Centered, and Smaller with a bit more shadow */}
      <div className="md:hidden fixed top-0 w-full flex justify-center items-center bg-white shadow-sm shadow-gray-500 py-2 ">
        <Image
          src="/images/navbar/logonav.webp"
          width={150} // Further reducing the size
          height={75}
          alt="Mobile Logo"
          className="object-contain"
        />
        {/* 2011 Text for Mobile */}
        <span className="absolute bottom-[10%] right-[25%] text-gray-600 text-sm font-bold">
        Since : 2011
        </span>
      </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 w-full bg-gradient-to-r from-[#E3F2FD] to-[#4A9BD3] shadow-lg rounded-t-xl">
        <div className="flex justify-around items-center py-3">
          ...
          {/* Our Services Icon with Dropdown */}
