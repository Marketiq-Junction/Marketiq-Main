...
const Approach = () => {
  ...
  return (
    <div className="relative bg-gray-50 px-6 sm:px-10 md:px-16 lg:px-20 py-10 lg:py-16 overflow-hidden mb-20 lg:mb-28">
      {/* Decorative Background */}
      <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[350px] sm:w-[450px] md:w-[500px] h-[350px] sm:h-[450px] md:h-[500px] bg-blue-100 rounded-full mix-blend-multiply filter blur-3xl opacity-30 pointer-events-none"></div>
      ...
      <div className="flex flex-col lg:flex-row gap-10 items-center relative z-10">
        ...
        <div className="lg:w-1/2">
          ...
          <motion.p
            className="text-base sm:text-lg lg:text-xl text-blue-500 italic mb-5"
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.5, delay: 0.4 }}
          >
          ...
          <ul className="space-y-2 sm:space-y-3">
            {bulletPoints.map((point, index) => (
              <motion.li
                ...
                transition={{ duration: 0.4, delay: 0.5 + index * 0.15 }}
              >
                <span className="text-blue-500 text-lg sm:text-xl mt-1">✔</span>
                {point}
              </motion.li>
            ))}
          </ul>
