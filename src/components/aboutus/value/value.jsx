    ...
    <section className="relative bg-gradient-to-b from-[#E6F9FD] to-white px-6 lg:px-16 py-16">
      ...
      >
        ...
        <p className="text-gray-600 text-base sm:text-lg">
          Our values define how we work and interact, fostering exceptional
          results.
        </p>
      </motion.div>
      ...
      {/* Center MJ Logo */}
      <motion.div
        className="z-20 bg-white border-8 border-[#4A9BD3] shadow-lg rounded-full w-44 sm:w-52 h-44 sm:h-52 flex items-center justify-center mx-auto"
        initial={{ scale: 0 }}
        animate={{ scale: 1 }}
        transition={{ duration: 0.6 }}
      >
      ...
      <div className="flex flex-wrap justify-center gap-6 mt-16">
        {values.map((value, index) => (
          <motion.div
            key={index}
            className="relative group w-72 sm:w-80 h-56 bg-white shadow-lg rounded-lg flex flex-col items-center justify-center gap-4 p-6 transition transform hover:scale-105 hover:rotate-1 hover:shadow-2xl"
            initial={{ opacity: 0, y: 50 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5, delay: index * 0.2 }}
          >
            <div className="absolute -top-12 w-20 h-20 sm:w-24 sm:h-24 bg-gradient-to-br from-[#4A9BD3] to-[#50C3C6] rounded-full flex items-center justify-center shadow-md border-4 border-white">
              {value.icon}
            </div>
            <div className="text-center mt-10">
      ...
      >
        <button
          ...
          className="bg-gray-800 text-white px-8 py-3 text-lg rounded-lg hover:bg-gray-900 transition duration-300 transform hover:scale-105"
        >
          Communicate with Us
        </button>
      </motion.div>
